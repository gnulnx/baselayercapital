...
show_averaged_output = False  # If True, show average results across all runs; if False, show last run only
show_failed_runs = False  # If True, print failed runs with Decay Rate == 0.2000 for debugging
...
starting_cash_reserves = 20_000  # Cash reserves to cover shortfalls in net cash (not invested unless needed)
...
vol_buy_threshold = -1.2  # Buy if price is 1.2 std dev below mean of lookback period (if tranches enabled)

btc_price_init = 104_896.3  # Initial BTC price
btc_growth_rate = 0.20  # Annual linear growth rate for BTC price (used for BTC LTV calculations)
...
ltv_buffer = 4  # Only stack loans if LTV < (target_ltv - ltv_buffer), stop if >= target_ltv

# Tax parameters
state_tax_rate = 0.05  # State tax rate (federal tax is computed monthly in the code)

dist_yield_low = 0.04  # Minimum monthly distribution yield (as a fraction, e.g., 0.04 = 4%)
...
# --- NAV Decay by Regime ---
bull_mean_decay = 0.01       # Bull: slight positive or flat NAV (simulate modest appreciation)
...
for _ in range(epochs):
    ...
    tranche_size = starting_cash_reserves / cash_tranche_count if cash_tranche_count > 0 else 0
    ...
    for month in range(1, months + 1):
        ...
        ltv = (loan_balance / (btc_price * btc_total)) * 100 if btc_price * btc_total > 0 else 0
        ...
        if net_cash < 0:
            ...
            if cash_reserves >= shortfall:
                ...
                if show_failed_runs:
                    ...
                    print(f"Decay Rate == 0.2000: {count_2000} months out of {total_months}")
        ...
        if (
            tranches_left > 0 and
            len(price_history) == cash_tranche_period and
            (month - last_buy_month) >= 1
        ):
            mean_price = sum(price_history) / cash_tranche_period
            std_price = (sum((p - mean_price) ** 2 for p in price_history) / cash_tranche_period) ** 0.5
...
ending_loan = ending_row['Loan Left'] if 'Loan Left' in ending_row else ending_row['Loan Bal']
...
print("\n--- Economic Summary (Final Month) ---")
print(f"ENDING BTC value: ${ending_row['BTC Val']}")
...
ax2 = ax1.twinx()
ax2.plot(df['Month'], df['Draw'], label='Draw (Income)', linestyle='--', color='tab:green')
ax2.set_ylabel('Draw (Income)', color='tab:green')
ax2.tick_params(axis='y', labelcolor='tab:green')

title = "Revenue vs Income Draw Over Time (Averaged Over Simulations)" if show_averaged_output else "Revenue vs Income Draw Over Time (Last Run)"
fig.suptitle(title)
fig.tight_layout()
